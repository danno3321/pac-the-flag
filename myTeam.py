...
def createTeam(firstIndex, secondIndex, isRed,
  ...
  agent1.setPartner(agent2)
  agent2.setPartner(agent1)
...
class ReactiveAgent(CaptureAgent):
  """
  ...
  def setPartner(self, ally):
    self.partner = ally

class Adapter(ReactiveAgent):
  """
  ...
  def getFeatures(self, gameState, action):
    ...
    partnerState = successor.getAgentState(self.partner.index)
    ...
    if len(invaders) > 0:
      ...
      partner_dists = [(self.partner).getMazeDistance(partnerPos, a.getPosition()) for a in invaders]
      newDists = []
      for i in range(0, len(invaders)):
        if (dists[i] <= partner_dists[i]):
          newDists.append(dists[i])
      if (len(newDists) > 0):
        features['invaderDistance'] = min(newDists)
        features['distanceToFood'] = 0
        if (myState.isPacman): features['reverse'] = -10000
      else: features['reverse'] = 0
    else: features['invaderDistance'] = 0
    ...
    if gameState.getAgentState(self.index).isPacman: features['fleeEnemy'] = 1.0 / close_dist
    else: features['distanceToGhost'] = 1.0 * close_dist

    return features

  def getWeights(self, gameState, action):
    return {'numInvaders': -10000, 'onDefense': (self.getScore(gameState) * 1), 'invaderDistance': -1000, 'stop': -10000, 'reverse': -1,
            'successorScore': 50, 'distanceToFood': -1.0, 'fleeEnemy': -1000.0, 'distanceToGhost': -1000.0}
